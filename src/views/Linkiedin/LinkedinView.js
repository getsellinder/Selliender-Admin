...
const LinkedinView = () => {
  ...
  const fetchAnalyses = async (page = 1) => {
    ...
    try {
      ...
      try {
        const userDetailsRes = await fetch(`/api/v1/user/details`, {
          method: "GET",
          headers: {
      ...
      const apiUrl = `/api/linked/disc/user/${userId}?page=${page}&limit=10`;
                        ...
                        {report.probabilityToPurchase.factors.map((f, i) => (
                          <tr key={i} className={i % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                            ...
                            <td className="px-3 py-3">
                              <span className={`px-2 py-1 rounded text-xs font-medium ${f.influence === 'High' ? 'bg-red-100 text-red-700' :
                  ...
                  {report.detailedPostMetrics.topPerformingPosts.map((post, index) => (
                    <div key={index} className="border border-gray-200 rounded p-3 bg-gray-50">
                      ...
                      <div className="flex flex-wrap gap-2 text-xs">
                        ...
                        {post.sentiment && (
                          <span className={`px-2 py-1 rounded ${post.sentiment === 'Positive' ? 'bg-green-100 text-green-700' :
                      ...
                      <tbody>
                        {report.confidenceExplanation.parameters.map((p, i) => (
                          <tr key={i} className={i % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                            ...
                            <td className="px-3 py-3">
                              <span className={`px-2 py-1 rounded text-xs font-medium ${parseInt(p.score) >= 8 ? 'bg-green-100 text-green-700' :
